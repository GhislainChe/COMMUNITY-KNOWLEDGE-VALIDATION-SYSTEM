...
router.patch("/admin/users/:userId/role", requireAuth, requireRole("ADMIN"), async (req, res) => {
  try {
    ...
    await pool.query("UPDATE users SET userRole=? WHERE userId=?", [userRole, userId]);
 ...
 */
router.patch("/admin/users/:userId/status", requireAuth, requireRole("ADMIN"), async (req, res) => {
  try {
    const userId = Number(req.params.userId);
    ...
    await pool.query("UPDATE users SET userStatus=? WHERE userId=?", [userStatus, userId]);
